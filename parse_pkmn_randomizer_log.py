...
class PkmnRandomizerLogParser():
    ...
    def get_wild_set_lines(self):
        return [ind for (ind, line) in enumerate(self.log_data) if self.is_wild_set_declaration(line)]

    def extract_wild_pkmn_from_lines(self, start_line_index: int, end_line_index: Optional[int] = None):
        wild_set_pkmn_regex = re.compile(r"^(?P<species>.*)(?=\s+Lv)\s+Lv((?P<lv>\d+)|s\s+(?P<lvrng>\d+\-\d+))", re.I)
        curr_ind = start_line_index + 1
        curr_match = wild_set_pkmn_regex.match(self.log_data[curr_ind])
        while curr_match is not None and (end_line_index is None or (end_line_index is not None and curr_ind < end_line_index)):
    ...
    def generate_location_level_ranges(self, wild_pkmn_data: list[dict]):
        ...
        for wild_pkmn in wild_pkmn_data:
            ...
            self.wild_pokemon.add(pkmn) # side-effect: this is just to make processing the list of wild pokemon easier later
    ...
    def get_wild_pokemon_data_from_logfile(self):
        ...
        for (wild_set_ind, wild_set_logstart_ind) in enumerate(wild_set_declarations):
            ...
            wild_pkmn_in_set = list(self.extract_wild_pkmn_from_lines(wild_set_logstart_ind, end_logindex))
            self.pkmn_by_location[wildset] = self.generate_location_level_ranges(wild_pkmn_in_set)
        
        return self.pkmn_by_location

    def get_locations_for_pkmn(self, species):
        if self.pkmn_by_location is None:
            raise LookupError(f"Wild pokemon location not found, cannot locate {species} in wild pokemon list.")
    ...
    def format_pokemon_stats(self, regex_match, include_locations = False):
            ...
            if isinstance(curr_pkmn_stats["items"], str):
                curr_pkmn_stats["items"] = list(map(str.strip, curr_pkmn_stats["items"].split(",")))

            # passed param-based inclusions -- dont include if flag not passed, so delete it
            if include_locations:
                curr_pkmn_stats["locations"] = list(self.get_locations_for_pkmn(curr_pkmn_species))
    ...
    def get_pokemon_stat_data_from_logfile(self, include_locations = True):
        ...
        stats_table_regex = re.compile(r"^\s*(?P<num>\d+)\|(?P<species>[^\|]+)\s*\|(?P<types>[^\|\s]+)\s*\|\s*(?P<hp>\d+)\|\s*(?P<atk>\d+)\|\s*(?P<def>\d+)\|\s*(?P<spatk>\d+)\|\s*(?P<spdef>\d+)\|\s*(?P<spd>\d+)\|(?P<ability1>[^\|]+)\|(?P<ability2>[^\|]+)\|(?P<items>[^\n]*)?$", re.I)
        while (stats_match := stats_table_regex.match(self.log_data[stats_table_index])) is not None:
            stats_table_index += 1
            (curr_pkmn_stats, curr_pkmn_species) = self.format_pokemon_stats(stats_match, include_locations=include_locations)
    ...
    def get_pokemon_stats(self, species):
        if self.pkmn_stats is None or species not in self.pkmn_stats:
            raise LookupError(f"Pokemon stats not found, cannot locate {species} in pokemon stat list.")
